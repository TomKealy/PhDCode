function [X, Z, vars_prob, varargout] = DADMM_AMP(n, vars_prob, vars_network,...
    varargin) 
...
for p = 1 : P
    rows = [offset:offset+vars_prob.m_p-1];
...
if EXISTS_X_OPT == 1
    if ERROR_FUN == 1
        ...
        [error_iterations_z, vars_prob] = error_fun_handler(X, x_opt, vars_prob);
    else
        error_iterations_z = norm(X{1}-x_opt)/norm(x_opt);
...
for color = 1 : num_colors
    ...
    for p = partition_colors{color}
        ...
        if ~Stop(p)              % Only compute if not has not terminated
            ...
            % Determine the sum of the Z's of the neighbors       
            sum_neighbs = zeros(n,1);                        
            for j = 1 : Dp
                sum_neighbs = sum_neighbs + Z{neighbs(j)};
            end
            sum_neighbs = sum_neighbs + Z{p};
            Ap = vars.Ap{p}; %pth row of A           
            
            %x-update 
            xp = (1/(Dp+1))*sum_neighbs;
            rp = Ap*xp - vars.bp{p} ;% + (1/vars_prob.m_p)*nnz(xp)*R{p};
            zp = xp - rho*shrinkage(Ap'*rp, vars_prob.beta);
...
if EXISTS_X_OPT == 1
    if ERROR_FUN == 1
        ...
        [new_error_x, vars_prob] = error_fun_handler(Z, x_opt, vars_prob);
    else
    ...
    ind_lesserror = (new_error_x < iter_for_errors(:,1));
...
function z = prox(x, gamma)
    n = length(x);
    z = zeros(size(x));
    for i=1:n
        if abs(x(i)) <= sqrt(2*gamma)
            z(i) = 0;
        else
            z(i) = x(i);
        end
