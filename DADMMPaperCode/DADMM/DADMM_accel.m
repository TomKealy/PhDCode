function [X, Z, vars_prob, varargout] = DADMM_accel(n, vars_prob, vars_network,...
    varargin) 
...
for k = 1 : MAX_ITER    
    ...
    for p = 1 : P
        ...
        if Stop(p) == 0  
            ...
            gp1 = (1+sqrt(1+4*gamma0{p}*gamma0{p}))/2;
            
            gamma = ((gamma0{p}-1)/(gp1));
            
            Z{p} = Z{p} + gamma*(Z{p} - Z_prev{p});
            
            %U{p} = U{p} + gamma*(U{p} - U_prev{p});
            
            T{p} = T{p} + gamma*(T{p} - T_prev{p});
...
for color = 1 : num_colors
    ...
    for p = partition_colors{color}
        ...
        if ~Stop(p)              % Only compute if not has not terminated
            ...
            xp = (Atb+rho*z_old - t - v);
            
            xp = q\xp;
            
            xp = relax*xp + (1-relax)*z_old;
            %z-update
