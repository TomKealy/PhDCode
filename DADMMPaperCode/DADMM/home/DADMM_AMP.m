function [X, Z, vars_prob, varargout] = DADMM_AMP(n, m, vars_prob, vars_network,...
    varargin) 
...
for color = 1 : num_colors
    ...
    for p = partition_colors{color}
        ...
        if ~Stop(p)              % Only compute if not has not terminated
            ...
            %x-update
            k = 1/vars.m;
            gammat = rho*k*norm(zp)^2/P;
            xp = shrinkage(xp + Ap'*zp + v, gammat);
            zp = vars.bp{p} - Ap*xp + k*nnz(xp)*zp;
