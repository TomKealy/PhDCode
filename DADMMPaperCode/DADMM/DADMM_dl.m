function [X, Z, D, vars_prob, varargout] = DADMM_dl(n, vars_prob, vars_network,...
    varargin) 
...
for p = 1 : P
    ...
    Atb{p} = Ap{p}'*bp{p};
    Dp = length(vars_network.neighbors{p});
    Q{p} = (Ap{p}'*Ap{p}+rho*(Dp+1)*speye(size(Ap{p},2)));
end
...
for k = 1 : MAX_ITER    
    ...
    for p = 1 : P
        ...
        if Stop(p) == 0  
            ...
            T{p} = T{p} + rho*(X{p} - Z{p});
            G = -Ap{p}'*(bp{p} - Ap{p}*vars.D{p}*Z{p})*Z{p}';
            mu = norm(G, 'fro')/norm(Ap{p}*G*Z{p});
            vars.D{p} = vars.D{p} + mu*G;
            vars.D{p} = sqrt(vars.s)*vars.D{p}/norm(vars.D{p}, 'fro');
            %vars.Ap{p} = vars.Ap{p}*D{p};
            
        end
    end
    vars.U = U;
    vars.T = T;
    %max_eig = max(abs(eig((Ap{1}*D{1})'*(Ap{1}*D{1}))));

    %vars.rho = nthroot(1/max_eig, 3);
...
for color = 1 : num_colors
    ...
    for p = partition_colors{color}
        ...
        if ~Stop(p)              % Only compute if not has not terminated
            ...
            %x-update
            q = (A'*A + rho*(Dp+1)*speye(size(Ap,2)));
            ...
            zp = prox(xp + (t/rho), (vars_prob.beta/P)/(rho));
