function [X, Z, vars_prob, varargout] = DADMM_mmv(n, m, vars_prob, vars_network,...
    varargin) 
...
for p = 1 : P
    ...
    U_aux{p} = zeros(n, m);
    L{p} = zeros(n, m);
    Ap{p} = vars_prob.A_BPDN;
    bp{p} = zeros(P, m);
    bp{p}(p,:) = vars_prob.b_BPDN(p,:); %giving each node a row of Y
    Atb{p} = Ap{p}'*bp{p};
    AtA{p} = zeros(n, n);
    AtA{p} = Ap{p}'*Ap{p};
end
...
for color = 1 : num_colors
    ...
    for p = partition_colors{color}
        ...
        if ~Stop(p)              % Only compute if not has not terminated
            ...
            %x-update
            q = (AtA+rho*(Dp+1)*speye(size(AtA,2)));
            ...
            Zp = wthresh(Xp + (T/rho), 's', (vars_prob.beta/P)/(rho));
...
function z = fast_sthresh(x,th)
    z = sign(x).*max(abs(x) - th,0);
end
