function [ w ] = slasso_eta_trick_squared_penalization( X, Y, XtY_over_n, XtX_over_n, G, D, lambda, params )

% SLASSO WITH SQUARED PENALIZATION SOLVED BY FIRST ORDER TECHNIQUE (see Appendix G in the paper)
%
% slasso_eta_trick_squared_penalization( X, Y, XtY_over_n, XtX_over_n, G, D, lambda, params )
%
% INPUTS:
%
...
while ( t <= max_it) && ( delta_cost > min_delta_cost), 
    ...
    Zeta = mexEta_update_for_regression_squared_penalty( w, G, D, epsilon/t );
    ...
    if ( mod(t,t0) == 0 ),
      ...
      cost = YtY_over_n / 2 + ( w'*XtX_over_n*w )/2 - XtY_over_n'*w  + lambda/2 * get_norm_evaluation( w, G, D )^2;
