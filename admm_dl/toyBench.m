...
lambdavec=[0.0001 0.001 0.01 0.1];

filename=[ comment 'Dictype-' type '-dim' int2str(dim) '-TailleDic' ...
    ...
    switch type
        ...
        case 'smooth'
            ...
            while i <= TailleDic
                ...
                long=floor(rand*dim/2);
                Xt(deb+1:min(deb+long,dim),i)=cos((deb+1:min(deb+long,dim))/(rand*50) + rand*pi);
                if norm(Xt(:,i))>0
                    i=i+1;
                end;
            end;
...
%    
    ...
    save(filename)
    remtime(filename,iter/nbitermax,['iteration ' num2str(iter)]);
