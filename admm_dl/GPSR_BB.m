...
% (to appear).
%
% Modified by A. Rakotomamonjy for Stopping criterion
%
% -----------------------------------------------------------------------
...
for cont_loop = 1:cont_steps
    ...
    while keep_going
        ...
        switch stopCriterion
            ...
            case 5
                %             s=tau*resid./norm(AT(resid),'inf');
                %             criterionDG=f - 0.5*s'*s - sum(y.*s);
                %             %criterionDG/f
                %
                %             derive=-K'*(yapp-K*alpha); % calcul du gradient
                derive=-AT(resid);
                Alpha=u-v;

                if size(tau,1)==1
                    tau=tau*ones(length(u),1);
                end;
                indpos=find(Alpha);
                indzero=find(Alpha==0);
                KKTconditionpos =  abs(abs(derive(indpos)) - tau(indpos));
                KKTconditionzero =  abs(derive(indzero)) -tau(indzero);
                indviolpos=find(KKTconditionpos>=tolA);
                indviolzero=find(KKTconditionzero>=tolA);



                keep_going = (~isempty(indviolzero) | ~isempty(indviolpos) );
            otherwise,
                error(['Unknown stopping criterion']);
