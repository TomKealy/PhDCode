...
cd(direct_networks);
load model.mat;     % File with randomly generated networks
cd(direct_current);
...
positions = randi(m,[1,100]);%generate random spikes for signal

%Add Gaussian, zero-mean noise to vector b
std_noise = 0.01*norm(b);
b = b% + std_noise*randn(m,1);

% For the groundtruth, we use the GPSR solver
[solution] = GPSR_BB(b, A_BP, beta, 'Verbose', 0, 'StopCriterion', 3, 'ToleranceA', 1e-6);

opts = spgSetParms('verbosity',0);
solution_spg = spg_bp(A_BP, b, opts);
