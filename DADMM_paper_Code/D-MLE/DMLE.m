function [X, vars_prob, varargout] = DMLE(n, vars_prob, vars_network,...
    varargin) 

% [X, vars_prob, varargout] = DMLE(n, vars_prob, vars_network,...
%  varargin)
%
...
%                       x in XP
%
% in a simulated distributed scenario using the D-MLE: 
%
%  [1] I. Schizas, A. Ribeiro, and G. Giannakis, "Consensus in Ad Hoc WSNs
%      With Noisy Links - Part I: Distributed Estimation of Deterministic 
%      Signals," IEEE Trans. Sig. Proc., Vol. 56, No. 1, 2008
%
% The size of the variable x is n. It is only required each function fp(.) 
% to be convex and each set Xp to be convex. D-MLE is oblivious to the 
% format of either fp or Xp. It only requires the user to provide a 
% function that solves, for each p,
%
%           minimize    fp(x) + v'*x + c||x||^2          (2)
%           subject to  x in Xp
%
% where the vector v and the scalar c are inputs given by D-MLE. The input 
% vars_prob, which is a struct, should contain the handler of the function 
% solving (2) in the field 'handler', i.e., 
% vars_prob.handler = function_handler. The remaining fields of vars_prob 
% are not used by D-MLE and can be used to store internal information for 
...
% of D-MLE.) The output xp is the solution of (2), and vars_prob is also
% returned because it might have changed (that depends on the user's 
% implementation).
%
% The remaining inputs and outputs of D-MLE are now explained.
%
% Inputs:
...
%                           criteria are turned off: D-MLE terminates 
...
%                  may relevant information, D-MLE returns it to the 
%                  user.
%
...
optargs = nargin - 3;
if optargs > 1
    error('Number of input arguments exceeds the maximum value. Please type ''help DMLE''.');
end
if optargs == 1
    if ~isstruct(varargin{1})
        error('Optional input should be a struct. Please type ''help DMLE''.');
    end
end
if ~isstruct(vars_prob)
    error('vars_prob (2nd argument of DMLE) should be a struct. Please type ''help DMLE''.');
end
if ~isfield(vars_prob, 'handler')
    error('vars_prob (2nd argument of DMLE) should contain the field ''handler''. Please type ''help DMLE''.');
end
if ~isstruct(vars_network)
    error('vars_network (3rd argument of DMLE) should be a struct. Please type ''help DMLE''.');
end
if sum(isfield(vars_network, {'P', 'neighbors'})) ~= 2
    error('Fieldnames of the struct vars_network are not correct. Please type ''help DMLE''.');
end
nout = max(nargout,1)-2;
if nout > 1
    error('Number of outputs is greater than the maximum. Please type ''help DMLE''.'); 
end
if nout == 1
    OPT_OUTPUT = 1; 
end
...
Eta = cell(P,1);                 % Cell (one/node) of dual variables
...
Z = cell(P,1);                   % Cell (one/node) with primal variable
...
% Struct with the internal variables of D-MLE. 
vars = struct( 'X', {X}, ...
    'X_prev', {X_prev}, ...
    'EPS', {EPS}, ...
...
for k = 1 : MAX_ITER    
    ...
    for p = 1 : P
        ...
        c = (Dp+1)*rho/2;
        v = Mu{p} - rho*u;
        [X{p}, vars_prob] = user_solver(p, v, c, X, vars_prob);
    end
    
    % Update the dual variables Eta and Mu
    for p = 1 : P 
        neighbs = neighbors{p};
        Dp = length(neighbs);
        
        % Determining
        % u_x = sum_{j in N_p {p}} x
        % u_z = sum_{j in N_p {p}} z
        u_x = zeros(n,1);
        u_z = zeros(n,1);
        ...
        u_x = u_x + X{p};
        u_z = u_z + Z{p};
        Eta{p} = Eta{p} + rho*(u_x - (Dp+1)*Z{p});
        Mu{p} = Mu{p} + rho*((Dp+1)*X{p} - u_z);
    end
    ...
    if EXISTS_X_OPT == 1
        ...
        error_iterations = [error_iterations, new_error, new_error]; %#ok<AGROW>
        ind_nonfilled = (iter_for_errors(:,2) == Inf);
        ind_lesserror = (new_error < iter_for_errors(:,1));
        intersect = ind_nonfilled & ind_lesserror;
        iter_for_errors(intersect,2) = 2*k;    % Two communication steps
    end
    ...
    vars.X_prev = X_prev;
    % Stopping criterion
    Stop = DMLE_stopping_criterion(Stop, vars_network, vars, vars_prob, k);
...
if OPT_OUTPUT == 1
    ...
    varargout{1} = struct('iterations', 2*k,...
        'stop_crit', stop_crit,...
...
function [Stop] = DMLE_stopping_criterion(Stop, vars_network, vars, vars_prob, k)
