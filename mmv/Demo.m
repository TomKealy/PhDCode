...
rho = nthroot(rho, 3);
lambda = 5*sqrt(2*log(195))/101;
beta = [1,1]/mean(abs(DigitalSamples(:)));

[B, cost] = lasso_admm_lucy(DigitalSamples, S, lambda, rho);
...
Adj = net.Adj;                   % Adjacency matrix
partition_colors = net.Partition;% Color partition of network
...
% title('Original signal'), xlabel('t')
% grid on
% figure,plot(t_axis,sigl,'r'), axis([t_axis(1) t_axis(end) 1.1*min(x) 1.1*max(x) ])
% title('Original noised signal'), xlabel('t')
% grid on
% figure, plot(t_axis,x_recn), axis([t_axis(1) t_axis(end) 1.1*min(x) 1.1*max(x) ]),
% grid on,
% title('Reconstructed signal'), xlabel('t')
