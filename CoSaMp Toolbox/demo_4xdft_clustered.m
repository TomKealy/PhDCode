%% Demonstration of Signal Space CoSaMP with 4x overcomplete DFT dictionary and with sparse coefficient vectors having clustered nonzeros
%
...
for trialNum = 1:numTrials
    for mIndex = 1:length(mVec)
        ...
        % Generate test signal with clustered nonzeros in coefficient
        % vector
        alpha = zeros(d,1);
        nonzeroStart = ceil(rand*(d-k+1));
        idx = [nonzeroStart:nonzeroStart+k-1];
        alpha(idx(1:k)) = randn(k,1) + 1i*randn(k,1);
        x = D*alpha;
    ...
    if mod(trialNum,25) == 0
        save demo_4xdft_clustered_results
    end    
end
...
if 0
    load demo_4xdft_clustered_results
    ...
    set(h,'FontSize',16);
    set(gca,'FontSize',20);
    plotpdftex(4,'demo_clustered.pdf', [1.5 1]); % have to enlarge legend box by hand before printing
